...
const expected_names = [_][]const u8{
    ...
    "core.session.session_log.test.native session checkpoint-only history derives a title after reopening unless explicitly named",
    ...
    "core.app.input_interrupt_runtime.test.interactive interrupt cancels without a native queue editor",
    ...
    "core.auth.codex_credential_broker.test.Codex credential broker admits only the pinned peer and instance nonce",
    "core.auth.codex_credential_broker.test.Codex credential broker rotates one generation and answers a stale generation with the newer lease",
    "core.auth.codex_credential_broker.test.Codex credential broker leases carry no refresh token or store representation",
    "core.auth.codex_credential_broker.test.Codex credential broker frames one persistent channel and fails a stalled frame",
    "core.auth.codex_credential_broker.test.Codex credential broker fails closed for host-managed and borrowed authority",
    "core.cli.cli_surface.test.hidden Codex credential activation stays all-or-none on agent-hosting launches",
    "core.app.app_entry_runtime.test.app entry starts the Codex credential broker before every other startup callback",
    ...
    "core.app.app_callbacks.test.agent deps forward app callbacks through core types",
    "acp.prompt.test.ACP recovery dependencies and config keep selected shape authority",
    ...
    "core.cli.cli_surface.test.resolved shape projection includes the base prompt and surface suppressions",
    "core.cli.cli_ask.test.ask usage and recovery dependencies keep selected shape and history authority",
    "core.cli.cli_ask.test.ask system override is included in the final shape identity",
    "core.cli.cli_ask.test.ask borrowed identity remains final for refreshable provider credentials",
    "core.cli.cli_ask.test.ask MCP runtime receives selected profile and shape configuration",
    "core.cli.cli_surface.test.ask launch retains state MCP permission context and native tool controls",
    "core.app.app_auth_runtime.test.interactive credential selection cannot escape the borrowed identity",
    "acp.server.test.ACP identity remains the only credential authority across routing and refresh",
    "acp.sessions.test.ACP launch MCP configuration follows the selected profile file",
