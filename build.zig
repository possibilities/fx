...
pub fn build(b: *std.Build) void {
    ...
    const fxnk_gate_tests = b.addTest(.{
        ...
        .filters = &.{
            ...
            "CLI surface uses the supplied command catalog for parsing usage and help",
            "global launch permission mode accepts exact auto for fresh and exact resume",
            "global launch permission mode rejects widening duplicates and non-launch commands",
            "process-only auto permissions file matches exact AgentWorkplace MCP names",
            ...
            "explicit launch auto defeats ambient and workspace yolo without persistence",
            "app bootstrap forwards explicit permission mode to lifecycle authority",
            ...
            "pending launch name transfers ownership once and clears without leakage",
            "startup picker cancellation retains launch name until persistence and a fresh session cannot inherit it",
            "interactive session resume uses the live transition and shared restore path",
            "launch name applies to the exact resumed target after new and does not leak",
            "validateSessionTitle accepts printable text and rejects malformed input",
            "renameActiveSession persists the title to the sidecar and session index",
            "fallback generated and manual titles share one durable metadata path",
            "global launch name parsing owns one normalized optional title",
            "global launch name parsing rejects duplicate missing and invalid values",
            "launch name is restricted to interactive and resume commands",
            "session display metadata validates explicit titles at the shared native boundary",
            "rendered top-level help is a complete CLI navigation page",
            "top-level help renders flags as compact aligned rows",
            "default top-level help styles fit the startup buffer",
            "launch admission final timestamp formatter emits strict UTC milliseconds",
            "launch admission final strict golden codec matches frozen schema v1 fixture",
            "launch admission final strict codec rejects noncanonical and malformed boundaries",
            "launch admission final digest helpers reproduce frozen launch and receipt authorities",
            "launch ledger pre-rename failures leave every durable boundary retryable",
            "launch ledger post-rename indeterminate failures recover every durable boundary",
            "launch ledger retries a matching digest with the current request identity",
            "launch ledger cancellation wins before admission and remains permanent",
            "launch ledger admission wins before cancellation and replays the original Turn",
            "launch ledger rejects conflicting digest and recovers a lost response",
            "launch ledger preserves active Conversation and every terminal outcome until exact ack",
            "launch ledger exact resume requires and retains the requested Conversation identity",
            "launch child runtime consumes a fresh Conversation reservation once and publishes active identity",
            "launch child runtime requires fresh reservation or the exact active Conversation",
            "launch child runtime durably decides the first Work-control prompt and replays its Turn",
            "native launch preparation reserves fresh identity and exact resume before exec",
            "external launch controls admit only one exact auto permission mode",
            "native prepared-fresh recovery reuses its reservation before any durable Conversation exists",
            "native prepared-fresh recovery resumes the durable active Conversation",
            "native supervisor retains exited signalled and exec-failed outcomes",
            ...
            "durable initial Work-control cancellation is permanent and exact admitted retries never enqueue twice",
            "durable initial Work-control recovery reuses an already-present Turn",
            "launch-correlated fresh session consumes its reservation once and publishes every active Conversation",
            "launch-correlated exact resume publishes the restored active Conversation",
            "structured subscription object schema validator enforces the local strict subset",
            "structured subscription schema compares JSON numbers without integer precision loss",
            "structured subscription schema rejects untyped keywords and bounds evaluation",
            "structured inference receipt ledger persists phases terminal replay and acknowledgement",
            "structured subscription inference enforces catalog order and exact tool-free request",
            "structured subscription inference replays terminals rejects conflicts and acknowledges idempotently",
            "structured subscription inference durably classifies refusal schema and provider failures",
            "structured subscription inference cancellation is durable before admission and provider terminal wins after admission",
            "structured subscription inference recovers admitted request without a second provider call",
            "structured subscription inference bounds provider identifiers and terminal persistence",
            "structured subscription CLI accepts one strict versioned inference frame",
            "structured subscription CLI rejects multiple missing-newline and oversized frames",
