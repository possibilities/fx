...
pub fn build(b: *std.Build) void {
    ...
    const fxnk_gate_tests = b.addTest(.{
        ...
        .filters = &.{
            ...
            "ACP command resolves selected native tools in invocation order",
            "app entry preserves ordered native tool selection across upgrade relaunch",
            "app entry relaunches only after teardown with the validated handoff",
            ...
            "agent deps forward app callbacks through core types",
            "ask usage and recovery dependencies keep selected shape and history authority",
            "ask system override is included in the final shape identity",
            "ask borrowed identity remains final for refreshable provider credentials",
            "ask MCP runtime receives selected profile and shape configuration",
            "ask launch retains state MCP permission context and native tool controls",
            "interactive credential selection cannot escape the borrowed identity",
            "ACP identity remains the only credential authority across routing and refresh",
            "ACP launch MCP configuration follows the selected profile file",
            "ACP recovery dependencies and config keep selected shape authority",
            ...
            "resolved shape projection includes the base prompt and surface suppressions",
