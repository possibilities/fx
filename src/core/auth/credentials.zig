    ...
    switch (provider) {
        .codex => {
            const credential = switch (mode) {
                .stored => try loadChatGptCredentialFromStore(alloc, oauth_transport.unavailable_provider, .stored, chatgpt_store),
                .refresh_if_needed => try loadChatGptCredentialFromStore(alloc, transport, .if_needed, chatgpt_store),
            };
...
pub fn resolveForProviderFromHome(
    ...
    switch (provider) {
        .codex => return .{ .credential = try loadChatGptCredentialFromHome(alloc, transport, mode, home) },
        .grok => return .{ .credential = try loadGrokCredentialFromHome(alloc, transport, mode, home) },
        .gateway => {},
    }
...
fn resolvePreferringFromHome(
    ...
    if (preferred) |source| {
        const chosen = loadPreferredSourceFromHome(alloc, transport, mode, source, home) catch |err| blk: {
            if (err == error.OutOfMemory) return err;
            debug_trace.logf("auth", "isolated preferred source load failed source={t} err={s}", .{ source, @errorName(err) });
            break :blk null;
        };
        if (chosen) |credential| return .{ .credential = credential };
        debug_trace.logf("auth", "isolated preferred source unavailable source={t}; using precedence", .{source});
    }
