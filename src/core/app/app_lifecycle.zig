...
pub fn loadStartupState(
    ...
    return loadStartupStateFromOwnedWorkspace(alloc, transport, secret_store, workspace_root, default_model, default_agent_step_limit, null, null, .refresh_if_needed);
}
...
pub fn loadCatalogStartupState(
    ...
    const workspace_root = try io_mod.realpathAlloc(alloc, ".");
    return loadStartupStateFromOwnedWorkspace(alloc, oauth_transport.unavailable_provider, secret_store, workspace_root, default_model, default_agent_step_limit, null, null, .stored);
}
...
fn loadStartupStateFromOwnedWorkspace(
    ...
    if (credential_mode) |mode| {
        const resolution = if (authorization_home) |home_dir|
