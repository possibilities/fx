...
pub fn Runtime(comptime App: type) type {
    return struct {
        ...
        fn switchProvider(
            ...
            if (!hostManagesAuth(app)) {
                credential = prepareProviderCredential(
                    app,
                    target,
                    ...
                    if (!borrowsAuthorization(app) and target == .codex and allow_login) {
                        try beginCodexSignInForProviderSwitch(app);
                        return;
                    }
                    if (!borrowsAuthorization(app) and target == .grok and allow_login) {
