...
fn runIfRequestedWithDeps(alloc: Allocator, args: []const [:0]const u8, cfg: Config, deps: RunDeps) !RunResult {
    var parsed_launch = parseInteractiveLaunch(alloc, args, cfg.command_catalog) catch |err| {
        ...
        try writer.writer.writeAll("usage: fx [--context-limit NAME=BYTES|off] [--add-dir PATH]... [--no-additional-dirs] [--provider <name>] [--model <id>] [--effort <level>] [--fast|--no-fast] [--system-prompt-file PATH] [--append-system-prompt-file PATH] <command>\n");
