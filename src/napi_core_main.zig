...
const ReadyNotifier = struct {
    ...
    fn notifyOpaque(raw: ?*anyopaque) void {
        const self: *ReadyNotifier = @ptrCast(@alignCast(raw.?));
        self.notify();
    }
...
const FetchBridge = struct {
    ...
    fn clearPendingRequest(self: *FetchBridge) void {
        ...
        std.crypto.secureZero(u8, @volatileCast(self.request.items));
    ...
    fn open(raw: ?*anyopaque, method: []const u8, url: []const u8, headers: []const u8, body: []const u8) !i32 {
        ...
        defer {
            const serialized = writer.writer.buffered();
        ...
        defer secret.zeroAndFree(std.heap.c_allocator, body_base64);
...
const Runtime = struct {
    ...
    catalog_context: host_model_catalog.Context = undefined,
    input: InputQueue = .{},
    output: OutputQueue = .{},
    credential: []u8,
    ...
    fn run(self: *Runtime) void {
        ...
        acp_server.runWithTransport(
            ...
            .{
                ...
                .libfx_gateway_model_catalog = host_model_catalog.provider(&self.catalog_context),
...
fn createRuntime(env: c.napi_env, options: c.napi_value) CreateError!*Runtime {
    ...
    runtime.codex_sessions.setNotifier(&runtime.ready, ReadyNotifier.notifyOpaque);
    ...
    runtime.catalog_context = .{ .transport = runtime.stream_context.transport };
